...
echo "🚀 setup script: This script will install everything required to run the Bookstore Sample App, and the Bookstore itself."

# Validate that the user is in the correct directory /solution
if [ "${PWD##*/}" != "knative-bookstore-e2e" ]; then
    echo "⚠️ Please run this script in the /knative-bookstore-e2e directory. Exiting..."
    exit
fi
echo "✅ You are in the correct directory: /knative-bookstore-e2e"
